...
def test_get_all_transactions():
    ...
    kwargs = {"status": 2, "fn_signature": ""}
    results = client.threaded_search(client.get_transactions, address, 0, to_block, 1, **kwargs)
    assert len(results) == 32

    kwargs = {"status": 0, "fn_signature": ""}
    results = client.threaded_search(client.get_transactions, address, 0, to_block, 1, **kwargs)
    assert len(results) == 8

    kwargs = {"status": 1, "fn_signature": ""}
    results = client.threaded_search(client.get_transactions, address, 0, to_block, 1, **kwargs)
...
def test_get_all_events():
    ...
    results = client.threaded_search(client.get_events,contract_address, from_block, to_block, 1, **kwargs)
